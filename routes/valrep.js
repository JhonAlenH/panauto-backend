...
router.route('/provider-notification').post((req, res) => {
    if(!req.header('Authorization')){ 
        res.status(400).json({ data: { status: false, code: 400, message: 'Required authorization header not found.' } })
        return;
    }else{
        operationValrepProviderNotification(req.header('Authorization'), req.body).then((result) => {
            if(!result.status){ 
                res.status(result.code).json({ data: result });
                return;
            }
            ...
            res.status(500).json({ data: { status: false, code: 500, message: err.message, hint: 'operationValrepProviderNotification' } });
        });
    }
});

const operationValrepProviderNotification = async(authHeader, requestBody) => {
    if(!helper.validateAuthorizationToken(authHeader)){ return { status: false, code: 401, condition: 'token-expired', expired: true }; }
    //if(!helper.validateRequestObj(requestBody, ['cproveedor'])){ return { status: false, code: 400, message: 'Required params not found.' }; }
    let cproveedor = requestBody.cproveedor ? requestBody.cproveedor: undefined;
    let query = await bd.providerNotificationValrepQuery(cproveedor).then((res) => res);
    if(query.error){ return { status: false, code: 500, message: query.error }; }
    let jsonArray = [];
    for(let i = 0; i < query.result.recordset.length; i++){
        jsonArray.push({ cproveedor: query.result.recordset[0].CPROVEEDOR, xnombre: query.result.recordset[0].XNOMBRE });
...
const operationValrepPlanRcvType = async(authHeader, requestBody) => {
    ...
    let jsonArray = [];
    for(let i = 0; i < query.result.recordset.length; i++){
        jsonArray.push({ cplan_rc: query.result.recordset[i].CPLAN_RC, xplan_rc: query.result.recordset[i].XDESCRIPCION, bactivo: query.result.recordset[i].BACTIVO });
    }
    return { status: true, list: jsonArray }
}
